...
Net::Net(std::vector<unsigned >& topologie) : topologie_(topologie)
{
    ...
    for (auto layer_num = 0; layer_num < layer_size; layer_num++)
    {
    ...
    /* get the index of the number of neurone in the next layer*/
        ...
        for (auto node_num = 0; node_num < topologie[layer_num]; node_num++)
...
void Net::feedForward(const std::vector<double> &inputVals)
{
    ...
    for (auto nodeNum = 0; nodeNum < inputVals.size(); nodeNum++)
        layers_[0][nodeNum].outputValue_set(inputVals[nodeNum]);
    
    /* feedForward on the other nodes */
    for (auto layerNum = 1; layerNum < layers_.size(); layerNum++) /*fix here remove - 1*/
    
    {
        ...
        for (auto nodeNum = 0; nodeNum < layers_[layerNum].size() - 1; nodeNum++)
        {
...
void Net::backProp(const std::vector<double> &targetVals)
{
   /*RMS error compute */
    ...
    mError_ = 0.0;
    for (auto numNode = 0; numNode < outputLayer.size(); numNode++)
    {
     ...
     */
    for (auto layerNum = layers_.size() - 2; layerNum > 0; --layerNum)
    {
        auto& layerHidden = layers_[layerNum];
        auto& layerNext = layers_[layerNum + 1];
        
        for (auto nodeNum = 0; nodeNum < layerHidden.size(); nodeNum++)
...
{
    res.clear();
    for (auto n = 0; n < layers_.back().size(); n++)
